...
import argparse
import os
from model import fine_model, build_generator, build_discriminator
...
if __name__ == '__main__':
    ...
    generator.summary()
    fine = fine_model(input_shape)
    if args.plot:
        from keras.utils import plot_model
        plot_model(fine, to_file='fine.png', show_shapes=True)

    x = Input(shape=input_shape)
    #x2 = Input(shape=input_shape)
    #x3 = Input(shape=input_shape)
    #x4 = Input(shape=input_shape)
    #inputs = [x1, x2, x3, x4]
    inputs = [x, x, x, x]
